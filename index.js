...
function validateConfig() {
  const { BOOKING_URL, USER_AGENT, CHECK_INTERVAL_MINUTES, HEALTHCHECKS_IO_TOKEN, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID  } = process.env;
  if (!BOOKING_URL | !CHECK_INTERVAL_MINUTES) {
    console.error('BOOKING_URL or CHECK_INTERVAL_MINUTES have not been set. Please set values in .env according to README.');
    process.exit(1);
  }

  if(!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    console.warn('TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID has not been set. You will receive no notifications for appointments.');
  }
