...
test('returns an empty array, when no appointment is available', () => {
  ...
  const html =
    fs.readFileSync(join(__dirname, 'fixtures', 'no-appointment-available.html'), { encoding: 'utf-8' });
...
test('returns one appointment, when one is available', () => {
  ...
  const html =
    fs.readFileSync(join(__dirname, 'fixtures', 'one-appointment-available.html'), { encoding: 'utf-8' });
...
test('returns multiple appointment, when multiple are available', () => {
  ...
  const html =
    fs.readFileSync(join(__dirname, 'fixtures', 'multiple-appointments-available.html'), { encoding: 'utf-8' });
